...
async function searchWallpapers(supabaseUrl, serviceKey, query, filters, page, limit, sortBy) {
    ...
    // Apply filters
    // Category filter: first lookup the category ID from slug, then filter by category_id
    if (filters?.category && filters.category !== 'all') {
        // Get category ID from slug
        const categoryResponse = await fetch(
            `${supabaseUrl}/rest/v1/categories?select=id&slug=eq.${filters.category}&is_active=eq.true&limit=1`,
            {
                headers: {
        ...
        if (categoryResponse.ok) {
            ...
            if (categoryData && categoryData.length > 0) {
                searchUrl += `&category_id=eq.${categoryData[0].id}`;
            }
        }
    }
    ...
    // showPremium: true = premium only (is_premium=true)
    // showPremium: false = all wallpapers (free + premium, no filter)
    // showPremium: undefined = premium only (is_premium=true) [default]
    if (filters?.showPremium === true) {
        searchUrl += `&is_premium=eq.true`;
    } else if (filters?.showPremium === false) {
        // showPremium: false - show all wallpapers (no premium filter)
        // This includes both free and premium wallpapers
        // No additional filtering needed
    } else {
        // Default: show premium only
        searchUrl += `&is_premium=eq.true`;
    }
    
    // Add video filter - check both live_video_url and live_enabled
    if (filters?.videoOnly === true) {
        searchUrl += `&or=(live_video_url.not.is.null,live_enabled.eq.true)`;
    }
