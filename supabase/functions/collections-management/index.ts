Deno.serve(async (req) => {
    ...
    try {
        // Cache request data to avoid "Body already consumed" error
        ...
        if (req.method === 'POST') {
            // Use cached requestData to avoid "Body already consumed" error
            let postData = requestData;
            ...
            const { name, slug, description, icon_name, cover_image_url, color_theme, is_seasonal, season_start_month, season_end_month, is_featured, is_active, sort_order } = postData;
        ...
        if (req.method === 'PUT') {
            // Use cached requestData to avoid "Body already consumed" error
            let putData = requestData;
            if (!putData || Object.keys(putData).length === 0) {
                try {
                    putData = await req.json();
                } catch (error) {
                    throw new Error('Invalid request body');
                }
            }
            const { id, name, slug, description, icon_name, cover_image_url, color_theme, is_seasonal, season_start_month, season_end_month, is_featured, is_active, sort_order, wallpaper_ids } = putData;
