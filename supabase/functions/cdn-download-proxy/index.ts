Deno.serve(async (req) => {
    ...
    try {
        ...
        // Get user profile for premium check (cached query)
        ...
        // Get wallpaper data (cached query)
        const wallpaperResponse = await fetch(`${supabaseUrl}/rest/v1/wallpapers?id=eq.${wallpaperId}&select=id,title,is_premium,download_url,resolution_1080p,resolution_4k,resolution_8k,download_count`, {
            headers: {
                'Authorization': `Bearer ${serviceRoleKey}`,
                'apikey': serviceRoleKey,
                'Accept': 'application/json'
        ...
        // Rate limiting check for free users (simplified for CDN compatibility)
        if (!isPremiumUser) {
        ...
        // Get the appropriate download URL based on resolution
        let downloadUrl;
        switch (resolution) {
            case '4k':
        ...
        // Handle Supabase Storage URLs with signed URL generation
            ...
            try {
                ...
                if (urlParts.length === 2) {
                    ...
                    if (signedUrlResponse.ok) {
                        ...
                        finalDownloadUrl = `${supabaseUrl}${signedData.signedURL}`;
                    }
                }
            } catch (storageError) {
        ...
        const fileResponse = await fetch(finalDownloadUrl, {
            headers: {
                'Accept': 'image/*,*/*',
                'User-Agent': 'Wallpaper-CDN-Proxy/1.0'
            }
        });
        ...
        const downloadFilename = `${cleanTitle}_${resolution}.${extension}`;
        ...
        const cdnHeaders = {
            ...
            'ETag': `"wallpaper-${wallpaperId}-${resolution}"`,
            'Last-Modified': new Date().toUTCString(),
            // Cloudflare specific headers
            'CF-Cache-Tag': `wallpaper,wallpaper-${wallpaperId}`,
        ...
        // For CDN compatibility, return appropriate HTTP status codes
        let status = 404;
